...
// ЗАМЕНИТЕ текущий интервал мониторинга на это:
setInterval(() => {
  const memory = process.memoryUsage();

  // Только логирование, без тяжелых операций
  if (memory.heapUsed > 300 * 1024 * 1024) { // 300MB вместо 500MB
    console.warn('🚨 HIGH MEMORY:', {
    ...
    if (quickCache.size > 50) {
      ...
      for (let [key, value] of quickCache.entries()) {
        ...
        if (quickCache.size <= 30) break; // Не очищать слишком много
      }
    }
  }
}, 600000); // 10 минут вместо 4
...
// ДОБАВЬТЕ лимит времени для Firebase операций
...
// 🔥 ИСПРАВЛЕНО: Оптимизированный мониторинг памяти с уменьшенной частотой и улучшенной очисткой
setInterval(() => {
  const memory = process.memoryUsage();

  // Только логирование без тяжелых операций
  if (memory.heapUsed > 300 * 1024 * 1024) { // 🔥 ИЗМЕНЕНО: 300MB вместо 500MB
    console.warn('🚨 ВЫСОКОЕ ПОТРЕБЛЕНИЕ ПАМЯТИ:', {
      heapUsed: Math.round(memory.heapUsed / 1024 / 1024) + 'MB',
      heapTotal: Math.round(memory.heapTotal / 1024 / 1024) + 'MB'
    });

    // Принудительный сбор мусора только если доступен
    if (global.gc) {
      global.gc();
    }
  }

  // 🔥 ИСПРАВЛЕНО: Оптимизированная очистка кэша
  cleanupCache();
}, 600000); // 🔥 ИЗМЕНЕНО: 10 минут вместо 4

// 🔥 ДОБАВЛЕНО: Функция оптимизированной очистки кэша
const MAX_CACHE_SIZE = 50;
const MAX_CACHE_AGE = 10 * 60 * 1000; // 10 минут

function cleanupCache() {
  if (quickCache.size > MAX_CACHE_SIZE) {
    const now = Date.now();
    let deletedCount = 0;
    ...
    for (let [key, value] of quickCache.entries()) {
      if (now - value.timestamp > MAX_CACHE_AGE) {
        quickCache.delete(key);
        deletedCount++;
        // 🔥 ИЗМЕНЕНО: Останавливаемся когда достигли 70% от максимального размера
        if (quickCache.size <= MAX_CACHE_SIZE * 0.7) break;
      }
    }
...
// 🔥 ИСПРАВЛЕНО: Уменьшены лимиты для экономии памяти
app.use(express.json({ limit: '2mb' })); // 🔥 ИЗМЕНЕНО: 2MB вместо 5MB
app.use(express.urlencoded({ extended: true, limit: '2mb' })); // 🔥 ИЗМЕНЕНО: 2MB вместо 5MB

const upload = multer({
  storage: multer.memoryStorage(),
  limits: {
    fileSize: 3 * 1024 * 1024, // 🔥 ИЗМЕНЕНО: 3MB вместо 5MB
    files: 2 // 🔥 ИЗМЕНЕНО: 2 файла вместо 3
  }
});
...
// 🔥 ДОБАВЛЕНО: Функция с таймаутом для операций
...
// 🔥 ДОБАВЛЕНО: Функция кэширования для Firebase с оптимизацией
async function getGroupWithCache(groupId) {
  const cacheKey = `group_${groupId}`;
  const cached = quickCache.get(cacheKey);
...
// 🔥 ИСПРАВЛЕНО: Оптимизированное middleware логирования
app.use((req, res, next) => {
  // Пропускать health-check запросы из логов
  if (req.url === '/health' || req.url === '/ping') {
    return next();
  ...
  try {
    ...
    for (const [userId, user] of Object.entries(users)) {
      ...
      if (name === fullName && role === 'педагог') {
        ...
        // Удаляем из всех групп
        const groupsSnap = await withTimeout(
...
app.post('/deleteChild', async (req, res) => {
  ...
  try {
    ...
    // 1. Получаем данные ребенка с таймаутом
    const childRef = db.ref(`users/${userId}/children/${childId}`);
    const childSnap = await withTimeout(
      childRef.once('value'),
      10000,
    ...
    const groupChildrenRef = db.ref(`groups/${groupId}/children`);
    const groupChildrenSnap = await withTimeout(
...
async function checkChatAccess(userId, chatId, isPrivate) {
  try {
    ...
    if (isPrivate) {
      ...
      // 🔥 ИСПРАВЛЕНО: Используем кэшированную версию с таймаутом
      const groupData = await getGroupWithCache(chatId);
...
async function isPrivateChatId(chatId) {
  try {
    if (chatId.includes('_')) {
      ...
      const privateChatRef = db.ref(`chats/private/${chatId}`);
      const privateSnap = await withTimeout(
...
app.post("/send-message", verifyToken, async (req, res) => {
  try {
    ...
    const senderSnap = await withTimeout(
    ...
    const isPrivateChat = await withTimeout(
    ...
    // 🔥 ИСПРАВЛЕНО: Отправка уведомлений в фоне без ожидания
    ...
    console.log("✅ Уведомления отправлены в фоне");
...
async function findParentsByGroupId(groupId) {
  try {
    ...
    const usersSnap = await withTimeout(
      db.ref('users').once('value'),
      15000,
      'users fetch for parents search'
    );
    const users = usersSnap.val() || {};
    const parents = [];
    const foundParentIds = new Set();
    ...
    for (const [userId, user] of Object.entries(users)) {
      if (user.role === "Родитель" && user.children) {
        ...
        const userDataSnap = await withTimeout(
          db.ref(`users/${userId}`).once('value'),
          3000,
          `user ${userId} details fetch`
        );
        const userData = userDataSnap.val() || {};
...
app.post("/send-event-notification", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const actualGroupName = await withTimeout(
...
// 🔥 ИСПРАВЛЕНО: Оптимизированный keep-alive с уменьшенной нагрузкой
const keepAlive = () => {
  setInterval(async () => {
    try {
      // 🔥 ИЗМЕНЕНО: Используем более легкий HTTP вместо HTTPS
      const http = require('http');
      const options = {
        hostname: process.env.RENDER_EXTERNAL_HOSTNAME || `firebase-admin-server-6e6o.onrender.com`,
        port: 80, // 🔥 ИЗМЕНЕНО: HTTP вместо HTTPS
        path: '/ping',
        method: 'GET',
        timeout: 3000 // 🔥 ИЗМЕНЕНО: 3 секунды вместо 5
      };
      ...
      req.on('error', (err) => {
        // 🔥 ИЗМЕНЕНО: Минимальное логирование для экономии памяти
        if (process.env.NODE_ENV === 'development') {
      ...
      // 🔥 ИЗМЕНЕНО: Без логирования для экономии памяти
    }
...
app.get("/info", (req, res) => {
  ...
  res.json({
    ...
    features: [
      ...
      "Таймауты для всех операций" // 🔥 ДОБАВЛЕНО
    ]
  });
});
...
const server = app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`✅ Keep-alive started (оптимизированный)`);
  console.log(`✅ Performance monitoring enabled`);
  console.log(`✅ Firebase caching enabled`);
  console.log(`✅ Timeout protection enabled`); // 🔥 ДОБАВЛЕНО
  keepAlive();
});
