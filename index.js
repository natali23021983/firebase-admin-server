  ...
  app.get("/info", (req, res) => {
    ...
    res.json({
      ...
      endpoints: {
        ...
        "POST /admin/remove-old-passwords":
          "Удаление старых паролей (требует токен)",
        ...
        // ЧАТ И СООБЩЕНИЯ
        "POST /send-message":
          "Сообщения с кэшированием пользователей (требует токен)",
        "POST /save-fcm-token": "Сохранение FCM токена (требует токен)",

        // УВЕДОМЛЕНИЯ
        "POST /send-event-notification":
          "Уведомления о событиях (требует токен)",

        // ФАЙЛЫ И S3
        "POST /generate-upload-url":
          "Генерация signed URL для загрузки (требует токен)",
