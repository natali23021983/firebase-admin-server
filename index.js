...
function startMainServer() {
  ...
  const THREAD_POOL_SIZE =
    process.env.UV_THREADPOOL_SIZE || (process.env.RENDER ? 64 : 128);
    ...
    set(key, value, ttl = 300000) {
      try {
        ...
        if (
          ...
          this.getMemoryUsage() > this.maxMemoryBytes * 0.8
        ) {
          this.evictOldest();
        }
    ...
    cleanup() {
      try {
        ...
        if (cleaned > 0 && process.env.NODE_ENV === "development") {
          console.log(
            "Автоочистка кэша: удалено " + cleaned + " устаревших записей",
          );
        }
  ...
  const MEMORY_LIMIT = process.env.RENDER
    ? 512 * 1024 * 1024
    : 1600 * 1024 * 1024;
  ...
  class AutomatedAdminSystem {
    ...
    async runAutomaticChecks() {
      try {
        ...
        const problems = checks
          .filter(
            (result) =>
              result.status === "fulfilled" && result.value.hasProblems,
          )
        ...
        if (problems.length > 0) {
          await this.notifyAdmin(
            "Обнаружены проблемы в системе: " + problems.join(", "),
          );
        }
      ...
      setInterval(
        async () => {
          try {
            ...
            if (archivedCount > 0) {
              await this.notifyAdmin(
                `Автоматически архивировано ${archivedCount} записей`,
              );
            }
          } catch (error) {
        ...
        async () => {
          try {
            ...
            if (result.success) {
              ...
              await this.notifyAdmin(
                `Создана резервная копия: ${result.fileName}`,
              );
              ...
              await this.notifyAdmin(
                `Ошибка создания резервной копии: ${result.error}`,
              );
            }
          } catch (error) {
        ...
        for (const adminUser of admins) {
          if (adminUser.fcmToken) {
            try {
              ...
              console.log(
                `Не удалось отправить уведомление администратору ${adminUser.userId}`,
              );
            }
          }
        }
    ...
    async checkDataIntegrity() {
      try {
        ...
        return {
          ...
          message:
            problems.length > 0
              ? `Найдено ${problems.length} проблем с данными`
              : "Данные в порядке",
        };
      } catch (error) {
        return {
          hasProblems: true,
          message: "Ошибка проверки целостности данных",
        };
      }
    }
    ...
    async checkArchiveNeeded() {
      try {
        ...
        const oldUsers = Object.values(users).filter(
          (user) => user.lastActivity && user.lastActivity < threeYearsAgo,
        ).length;
      ...
      try {
        ...
        return {
          ...
          groups: {
            ...
            withTeachers: Object.values(groups).filter(
              (group) =>
  ...
  async function getUserWithCache(userId) {
    ...
    try {
      ...
      console.error(
        "Ошибка получения пользователя " + userId + ":",
        error.message,
      );
      return null;
    }
  ...
  async function getNewsWithCache(groupId) {
    ...
    try {
      ...
      console.error(
        "Ошибка получения новостей группы " + groupId + ":",
        error.message,
      );
      return {};
    }
        ...
        if (name === fullName && role === "родитель") {
          ...
          if (user.children) {
            ...
            for (const [childId, child] of Object.entries(user.children)) {
              if (child.group) {
                await db
                  .ref(`groups/${child.group}/children/${childId}`)
                  .remove();
          ...
          try {
            ...
            console.log(
              "Пользователь не найден в Auth, пропускаем:",
              authError.message,
            );
          }
  ...
  app.post("/deleteChild", async (req, res) => {
    ...
    try {
      ...
      if (groupId) {
        ...
        for (const [groupChildId, groupChildName] of Object.entries(
          groupChildren,
        )) {
      ...
      if (err.message.includes("timeout")) {
        return res
          .status(408)
          .json({ error: "Операция заняла слишком много времени" });
      }
  ...
  app.post("/update-user", async (req, res) => {
    ...
    try {
      ...
      if (!fullName || !newEmail)
        return res
          .status(400)
          .json({ error: "fullName и newEmail обязательны" });
      ...
      if (keys.length > 1)
        return res
          .status(400)
          .json({ error: "Найдено несколько пользователей с таким именем" });
  ...
  app.put("/news", verifyToken, async (req, res) => {
    ...
    try {
      ...
      if (!newsId || !groupId || !title || !description) {
        return res
          .status(400)
          .json({ error: "newsId, groupId, title и description обязательны" });
      }
      ...
      if (oldNews.authorId !== authorId) {
        return res
          .status(403)
          .json({ error: "Нет прав для редактирования этой новости" });
      }
  ...
  app.get("/news", verifyToken, async (req, res) => {
    ...
    try {
      ...
      console.log(
        `GET /news запрос для группы: ${groupId}, пользователь: ${userId}`,
      );
      ...
      console.log(
        `Возвращаем ${newsArray.length} новостей для группы ${groupId}`,
      );
  ...
  app.post("/generate-upload-url", verifyToken, async (req, res) => {
    ...
    try {
      ...
      if (!fileName || !fileType) {
        return res
          .status(400)
          .json({ error: "fileName и fileType обязательны" });
      }
      ...
      if (
        ...
        !finalFileName.toLowerCase().endsWith(fileExtension.toLowerCase())
      ) {
    ...
    try {
      ...
      if (isPrivate) {
        ...
        if (group) {
          ...
          if (group.teachers) {
            for (const [teacherId, teacherName] of Object.entries(
              group.teachers,
            )) {
    ...
    try {
      ...
      for (const [userId, user] of Object.entries(users)) {
        if (user.children && !foundParentIds.has(userId)) {
          for (const [parentChildId, parentChildData] of Object.entries(
            user.children,
          )) {
            if (parentChildData && parentChildData.fullName) {
              const normalizedName = parentChildData.fullName
                .trim()
                .toLowerCase();
      ...
      if (process.env.NODE_ENV === "development") {
        console.log(
          `Данные родителей группы ${groupId} сохранены в кэш (${parents.length} родителей)`,
        );
      }
  ...
  async function sendNotificationsParallel(
    recipients,
  ...
  async function sendEventNotificationsOptimized({
    ...
    try {
      const parentsWithTokens = parents.filter(
        (parent) => parent.fcmToken && parent.fcmToken.trim() !== "",
      );
      ...
      return await sendNotificationsParallel(
        parentsWithTokens,
        createMessagePayload,
        20,
      );
  ...
  app.post("/send-event-notification", verifyToken, async (req, res) => {
    ...
    try {
      ...
      const notificationBody = formatEventNotification(
        title,
        time,
        place,
        actualGroupName,
  ...
  app.get("/warmup-cache", async (req, res) => {
    ...
    console.log(
      `[${requestId}] GET /warmup-cache - Запрос от ${req.ip || "unknown"}`,
    );
    ...
    if (timeSinceLastWarmup < WARMUP_COOLDOWN) {
      const remainingCooldown = Math.ceil(
        (WARMUP_COOLDOWN - timeSinceLastWarmup) / 1000,
      );
      console.log(
        `[${requestId}] Слишком частый запрос, cooldown: ${remainingCooldown}с`,
      );
      const stats = quickCache.getStats();
      return res.json({
        success: true,
    ...
    try {
      ...
      setTimeout(async () => {
        try {
          ...
          await Promise.allSettled([getGroupsStructureWithCache()]);

          const warmupTime = Date.now() - warmupStart;
          const finalStats = quickCache.getStats();

          console.log(
            `[${requestId}] Фоновый разогрев завершен за ${warmupTime}ms`,
          );
          console.log(
            `[${requestId}] Кэш: ${finalStats.size} записей, HitRate: ${finalStats.hitRate}`,
          );
        } catch (error) {
          console.error(
            `[${requestId}] Ошибка фонового разогрева:`,
            error.message,
          );
        } finally {
  ...
  app.get("/metrics", (req, res) => {
    ...
    const errorRate =
      global.performanceMetrics.requests > 0
        ? (
            (global.performanceMetrics.errors /
              global.performanceMetrics.requests) *
            100
  ...
  app.get("/info", (req, res) => {
    ...
    res.json({
      ...
      endpoints: {
        ...
        "GET /metrics": "Детальные метрики производительности",

        // КЭШ И ОПТИМИЗАЦИЯ
        "GET /cache-stats": "Детальная статистика кэша",
        "POST /warmup-cache": "Разогрев кэша",
        "POST /reset-cache": "Принудительный сброс всего кэша",

        // АДМИНИСТРИРОВАНИЕ
        "GET /admin/simple-status": "Статус системы (требует токен)",
        "POST /admin/remove-old-passwords": "Удаление старых паролей (требует токен)",
        "POST /admin/migrate-passwords": "Миграция паролей (требует токен)",

        // УПРАВЛЕНИЕ ПОЛЬЗОВАТЕЛЯМИ
        "POST /deleteUserByName": "Удаление пользователя по имени",
        "POST /deleteChild": "Удаление ребенка по userId и childId",
        "POST /update-user": "Обновление email пользователя",

        // НОВОСТИ И СОБЫТИЯ
        "GET /news": "Получение новостей с кэшированием",
        "POST /news": "Создание новости (требует токен)",
        "PUT /news": "Редактирование новости (требует токен)",
        "POST /deleteNews": "Удаление новости (требует токен)",

        // ЧАТ И СООБЩЕНИЯ
        "POST /send-message": "Сообщения с кэшированием пользователей (требует токен)",
        "POST /save-fcm-token": "Сохранение FCM токена (требует токен)",

        // УВЕДОМЛЕНИЯ
        "POST /send-event-notification": "Уведомления о событиях (требует токен)",

        // ФАЙЛЫ И S3
        "POST /generate-upload-url": "Генерация signed URL для загрузки (требует токен)",

        // АВТОМАТИЧЕСКИЕ СИСТЕМЫ
        "GET /wake-up": "Пробуждение сервера и проверка зависимостей",
        "GET /keep-alive": "Проверка доступности сервера",

        // ЭКСПОРТ ДАННЫХ
        "POST /admin/create-backup-now": "Ручное создание бэкапа (если добавлен)",
        "GET /test-s3-connection": "Проверка подключения к S3 (если добавлен)",
      },
      automatic_systems: {
        monitoring: "24 часа - проверка здоровья системы",
        archiving: "30 дней - архивация старых данных",
        backups: "7 дней - резервное копирование в S3",
        notifications: "автоматические уведомления администраторам"
      }
    });
  });
  ...
  app.get("/admin/simple-status", verifyToken, async (req, res) => {
    try {
      ...
      const memoryPercent = Math.round(
        (memoryUsage.heapUsed / MEMORY_LIMIT) * 100,
      );
  ...
  const server = app.listen(PORT, "0.0.0.0", () => {
    ...
    console.log(
      `Лимит кэша: ${quickCache.maxSize} записей, ${Math.round(quickCache.maxMemoryBytes / 1024 / 1024)}MB`,
    );
